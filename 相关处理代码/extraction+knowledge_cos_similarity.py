...
# 计算摘要_embedding的余弦相似度
text_extraction_embeddings_matrix = np.vstack(textEmbedding['摘要_embedding'])
knowledge_extraction_embeddings_matrix = np.vstack(knowledgeEmbedding['摘要_embedding'])
cosine_sim_extraction = cosine_similarity(knowledge_extraction_embeddings_matrix, text_extraction_embeddings_matrix)

# 计算aggregated_embedding的余弦相似度
text_aggregated_embeddings_matrix = np.vstack(textEmbedding['aggregated_embedding'])
knowledge_aggregated_embeddings_matrix = np.vstack(knowledgeEmbedding['aggregated_embedding'])
cosine_sim_aggregated = cosine_similarity(knowledge_aggregated_embeddings_matrix, text_aggregated_embeddings_matrix)

# 聚合两个表的嵌入向量
knowledgeEmbedding['combined_embedding'] = knowledgeEmbedding['摘要_embedding'] + knowledgeEmbedding['aggregated_embedding']
textEmbedding['combined_embedding'] = textEmbedding['摘要_embedding'] + textEmbedding['aggregated_embedding']

# 计算聚合后的余弦相似度
knowledge_combined_embeddings_matrix = np.vstack(knowledgeEmbedding['combined_embedding'])
text_combined_embeddings_matrix = np.vstack(textEmbedding['combined_embedding'])
